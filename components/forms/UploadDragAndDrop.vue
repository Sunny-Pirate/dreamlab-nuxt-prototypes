...
<template>
  <div class="card">
    <Toast/>
    <FileUpload name="demo[]" url="/api/upload" @upload="onTemplatedUpload($event)" :multiple="true" accept="image/*" :maxFileSize="1000000"
      ...
      <template #header="{ chooseCallback, uploadCallback, clearCallback, files }">
        <div class="flex flex-wrap justify-between items-center flex-1 gap-2">
          ...
          <ProgressBar :value="totalSizePercent" :showValue="false"
                       :class="['md:w-[20rem] h-4 w-full md:ml-auto', { 'exceeded-progress-bar': totalSizePercent > 100 }]"
        ...
        <div v-if="files.length > 0">
          ...
          <div class="flex flex-wrap p-0 sm:p-5 gap-5">
            <div v-for="(file, index) of files" :key="file.name + file.type + file.size"
                 class="card m-0 px-6 flex flex-col border border-surface-300 dark:border-surface-700 items-center gap-3">
              <div>
              ...
              <Button icon="pi pi-times" @click="onRemoveTemplatingFile(file, removeFileCallback, index)" outlined rounded
                      severity="danger"/>
            </div>
          </div>
        </div>
